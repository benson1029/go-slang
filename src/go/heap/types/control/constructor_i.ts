...
import { TAG_CONTROL_constructor_i } from "../tags";
import { UserType } from "../user/type";

class ControlConstructorI extends HeapObject {
  public get_type(): UserType {
    return auto_cast(this.heap, this.get_child(0)) as UserType;
  }

  public get_number_of_arguments(): number {
    return (auto_cast(this.heap, this.get_child(1)) as PrimitiveInt32).get_value();
  }

  public static allocate(heap: Heap, type: UserType, num_args: number): number {
    const address = heap.allocate_object(TAG_CONTROL_constructor_i, 1, 2);
  ...
  public stringify_i(): string {
    let result = "";
    result += this.address.toString() + " (constructor_i): ";
