...
import { TAG_CONTROL_member_address_i } from "../tags";

class ControlMemberAddressI extends HeapObject {
  public get_member_name_address(): ComplexString {
    if (this.get_tag() !== TAG_CONTROL_member_address_i) {
  ...
  public static allocate(heap: Heap, member: ComplexString): number {
    const address = heap.allocate_object(TAG_CONTROL_member_address_i, 1, 1);
